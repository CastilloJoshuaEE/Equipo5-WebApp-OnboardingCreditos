...
class ProcesarFirma{
    constructor(
        ...
        wordService, 
        notificacionService,
        // Agregar los casos de uso de notificaciones como dependencias
        notificarFirmaSolicitanteCompletadaUseCase,
        notificarFirmaOperadorCompletadaUseCase,
        notificarFirmaCompletadaUseCase
    ){
        this.firmaDigitalRepository = firmaDigitalRepository;
        this.wordService = wordService;
        this.notificacionService = notificacionService;
        this.notificarFirmaSolicitanteCompletada = notificarFirmaSolicitanteCompletadaUseCase;
        this.notificarFirmaOperadorCompletada = notificarFirmaOperadorCompletadaUseCase;
        this.notificarFirmaCompletada = notificarFirmaCompletadaUseCase;
    }
    ...
    async execute(firma_id, {firma_data, tipo_firma}, usuario){
        ...
        // Procesar según el tipo de firma - AHORA USANDO LOS CASOS DE USO
        if(tipo_firma === 'solicitante'){
    ...
    async procesarFirmaOperador(firmaId, firma){
        try{
            ...
            if(firmaActual.fecha_firma_solicitante && firmaActual.fecha_firma_operador){
                ...
                // USAR EL CASO DE USO EN LUGAR DE this.notificacionService
                if (this.notificarFirmaOperadorCompletada) {
                    await this.notificarFirmaOperadorCompletada.execute(firma.contrato_id);
                }
            }
        } catch (error){
    ...
    async marcarFirmaCompleta(firmaId, firma){
        try{
            ...
            // USAR EL CASO DE USO EN LUGAR DE this.notificacionService
            if (this.notificarFirmaCompletada) {
