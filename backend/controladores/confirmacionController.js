...
class ConfirmacionController {
  ...
  static async enviarEmailBienvenidaDespuesConfirmacion(email, nombre, rol) {
    try {
      ...
      const resultado = await ConfirmacionController.enviarEmailBienvenida(email, nombre, rol);
  ...
  static async confirmarEmail(req, res) {
    try {
      ...
      try {
        ...
        await ConfirmacionController.enviarEmailBienvenidaDespuesConfirmacion(
          usuario.email, 
   ...
   * @param {import('express').Response} res 
   */
  static async reenviarConfirmacion(req, res) {
    try {
      ...
      const emailResult = await ConfirmacionController.enviarEmailConfirmacionCuenta(
        usuario.email, 
        usuario.nombre_completo, 
        usuario.id
      );
   ...
   * @param {import('express').Response} res 
   */
  static async estadoConfirmacionEmail(req, res) {
    try {
  ...
  static async enviarEmailBienvenida(email, nombre, rol) {
    try {
      ...
      const plantilla = ConfirmacionController.crearPlantillaBienvenida(nombre, rol);
      const resultado = await ConfirmacionController.enviarEmailBrevo(email, plantilla.asunto, plantilla.html, plantilla.texto);
...
Sistema de Créditos`
   ...
   * @returns {Promise<Object>}
   */
  static async verificarServicioEmail() {
    return await verificarConexionBrevo();
  }
}
