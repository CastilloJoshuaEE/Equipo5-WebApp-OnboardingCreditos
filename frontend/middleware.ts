...
export async function middleware(request: NextRequest) {
  ...
  const isDashboardPage =
    request.nextUrl.pathname.startsWith("/solicitante") ||
    request.nextUrl.pathname.startsWith("/operador");
  ...
  if (token && isAuthPage) {
    const dashboardPath =
      token.rol === "solicitante" ? "/solicitante" : "/operador";
    return NextResponse.redirect(new URL(dashboardPath, request.url));
  }

  // Protección de rutas de dashboard por rol
  if (token && isDashboardPage) {
    const isSolicitantePath =
      request.nextUrl.pathname.startsWith("/solicitante");
    const isOperadorPath = request.nextUrl.pathname.startsWith("/operador");
